...
func (store *SQLStore) TransferTx(ctx context.Context, arg TransferTxParams) (TransferTxResult, error) {
	...
	err := store.execTx(ctx, func(q *Queries) error {
		...
		// one of the method: get account -> update its balance
		// this method must use FOR UPDATE, but it will still error because of deadlock
		// it will appear deadlock because of the foreign key constraint
		// but remove the foreign key is still not a good choice
		// the best choice is directly update
